...
public partial class AssetPlacerPlugin : ContextlessPlugin
{
	...
	private SpawnParentSelection _spawnParentSelection;
	...
	protected override void _Init()
	{
		...
		_spawnParentSelection = new SpawnParentSelection();
		AddChild(_spawnParentSelection);
		_spawnParentSelection.Init(GetEditorInterface());
	...
	protected override void _ProcessUpdate(double delta)
	{
		...
		if (_spawnParentSelection.SpawnParent == null)
		{
		...
		if (IsMouseOverValidFocused3dViewport() && !rmbPressed)
		{
			...
			if (positionInfo.posValid)
			{
				if (parentOrNull == null)
				{
					...
					_spawnParentSelection.SpawnParent.AddChild(_assetPalette.Hologram); // now hologram has a parent
	...
	private void ProcessRotatingInstance()
	{
		...
		if (!lmbPressed)
		{
			...
			undo.CreateAction("Place asset");
			undo.AddDoMethod(this, nameof(PlaceAssetAction), _assetPalette.LastPlacedAsset, _spawnParentSelection.SpawnParent, _assetPalette.LastPlacedAsset.Name, transform);
			undo.AddUndoMethod(this, nameof(UndoPlaceAsset), _assetPalette.LastPlacedAsset, _spawnParentSelection.SpawnParent);
	...
	private void ProcessPaintingHologram()
	{
		...
		if (!lmbPressed)
		{
			...
			// undoRedo system
			var undo = GetUndoRedo();
			undo.CreateAction($"Place {_paintingAssets.Count} assets");
			undo.AddDoMethod(this, nameof(PlaceAssetsAction), list, _spawnParentSelection.SpawnParent, asset.Name);
			undo.AddUndoMethod(this, nameof(UndoPlaceAssets), list, _spawnParentSelection.SpawnParent);
	...
	protected override bool _Forward3DViewportInput(Viewport viewport, InputEvent @event)
	{
		...
		if (_processState == ProcessState.PlacingHologram && _spawnParentSelection.SpawnParent != null)
		{
	...
	{
		var holoParent = _assetPalette.Hologram.GetParentOrNull<Node3D>();
