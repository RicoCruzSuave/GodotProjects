...
public partial class AssetPreviewGenerator : Node
{
        ...
        Texture2D brokenTexture, Asset3DData.PreviewPerspective previewPerspective)
    {
        if (resource is PackedScene scene)
        {
            ...
            _pendingLoadActions.Add(new LoadData(onPreviewLoaded, scene, size, perspective));
        }
        else
        {
        ...
        foreach (var loadAction in _pendingLoadActions)
        {
            if (vpMaybeAvailable && loadAction.waiting)
            {
                ...
                if (availableViewport != null)
                {
                    ...
                    availableViewport.SetPreviewNode(loadAction.sceneResource, loadAction.previewPerspective);
    ...
    private void OnLoadFinished(LoadData loadData)
    {
        ...
        loadData.action.Call(loadData.sceneResource.ResourcePath, texture);
        ...
        _textureCache.AddToCache(GetCacheKey(loadData.sceneResource, loadData.previewPerspective), texture);
    }

    public List<string> HandledFiles()
    {
        return new List<string>() { "glb", "tscn", "gltf", "dae", "fbx", "obj", "blend" };
    }
